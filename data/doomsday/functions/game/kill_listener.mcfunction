...
#witch
execute if score @s[tag=ddy_witch,nbt={Inventory:[{tag:{ddy_item:4b,item_id:1b}}]}] ddy_witch_tag_count matches 20.. run tellraw @s "{\"text\":\"§6[末日餘燼] §武器已升級。\"}"

execute if score @s[tag=ddy_witch,nbt={Inventory:[{tag:{ddy_item:4b,item_id:1b}}]}] ddy_witch_tag_count matches 20.. run function doomsday:profession_bringer/witch/weapon_update1
